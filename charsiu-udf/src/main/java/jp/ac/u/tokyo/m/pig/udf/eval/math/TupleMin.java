...
/**
 * From Bag of the comparable value corresponding to the order of Bag and the Bag, a comparable value outputs Tuple becoming minimum in a mass in Bag. <br>
 * <br>
 ...
 * <br>
 * Bag と、その Bag の順序に対応した比較可能値の Bag から、比較可能値が最小となる Tuple を Bag にまとめて出力します。 <br>
 * <br>
 * この実装クラスでは 数値 の比較を担当します。 <br>
 * bytearray は double に変換し、このクラスで比較します。 <br>
 * それ以外の比較可能値については getArgToFuncMapping() に記述するクラスで担当します。 <br>
 */
public class TupleMin extends EvalFunc<DataBag> {
	...
	public DataBag exec(Tuple aInput) throws IOException {
		...
		while (tComparableValueBagIterator.hasNext()) {
			...
			if (tCurrentValue == null)
				continue;
			// add a tuple to tProtoBag if the same as existing MinValue
			// 現 MinValue と同じなら、タプルを tProtoBag に追加
			if (tMinValue.equals(tCurrentValue)) {
				tProtoBag.add(tCurrentTargetTuple);
			}
			// clear tProtoBag if smaller than existing MinValue and add a tuple to tProtoBag
			// 現 MinValue より小さいなら、tProtoBag をクリアし、タプルを tProtoBag に追加
			else if (tMinValue > tCurrentValue) {
	...
	public List<FuncSpec> getArgToFuncMapping() throws FrontendException {
		...
		addFuncSpec(tFuncList, DataType.CHARARRAY, TupleStringMin.class.getName());
		return tFuncList;
	}
